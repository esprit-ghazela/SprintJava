...
public class LivraisonController implements Initializable {
    ...
    private void ajouterlivraison(ActionEvent event) throws SQLException {
        ...
        if (adrl.getText().isEmpty()) {
            if (etall.getText().isEmpty()) {
                errors_adr.setTextFill(Color.TOMATO);
                errors_adr.setText("Etat ne peut pas étre un champ vide");
            ...
            try {
                ...
                new Alert(Alert.AlertType.INFORMATION, "Livraison ajouté").show();

                clearFields();
                // AfficherCommannde) ;
                try {
                    BorderPane pane = FXMLLoader.load(getClass().getResource("/views/Produits.fxml"));
